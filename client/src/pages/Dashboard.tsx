...
export default function Dashboard() {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const res = await fetch(
        `/api/stock/${encodedSymbol}?range=6mo&interval=1d`,
      );
  ...
  const isKLSE =
    ...
    symbol.includes(".KL") ||
    symbol.includes("MYX") ||
    stockData?.exchange === "KLS" ||
    stockData?.currency === "MYR";
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-8 relative z-10 space-y-12">
        <div className="flex items-center justify-between bg-card/30 border border-primary/10 rounded-lg px-4 py-2.5">
          <div className="flex items-center gap-3 text-sm text-muted-foreground">
            ...
            <span className="font-light" data-testid="text-session-email">
              {email}
            </span>
            <span className="text-primary/30">|</span>
            <span
              className={`flex items-center gap-1.5 font-mono text-xs ${timeLeft < 120000 ? "text-red-400" : "text-muted-foreground"}`}
            ...
            <span className="text-primary/30">|</span>
            <span
              className="flex items-center gap-1.5 text-[10px] text-muted-foreground/90"
              data-testid="text-ai-model"
            >
        ...
        <header className="flex flex-col md:flex-row justify-between items-center gap-8 border-b border-primary/20 pb-8">
          <div className="text-center md:text-left space-y-3">
            <div className="flex items-center justify-center md:justify-start gap-2 text-primary mb-1">
              ...
              <span className="text-[10px] uppercase tracking-[0.25em] font-medium opacity-80">
                {t("privateClients")}
              </span>
            </div>
            <h1
              className="text-4xl md:text-5xl font-serif font-medium tracking-tight text-foreground"
          ...
          <div className="flex flex-col gap-2 w-full md:w-auto">
            <form
              ...
              className="flex gap-0 shadow-2xl shadow-black/40 rounded-lg overflow-hidden border border-primary/20 group focus-within:border-primary/50 transition-colors w-full md:w-auto"
            >
              ...
              <Button
                type="submit"
                className="h-12 px-8 rounded-none bg-primary text-primary-foreground font-medium tracking-widest hover:bg-primary/90 transition-all cursor-pointer"
            ...
            <p
              ...
              data-testid="text-analyze-hint"
            >
              {t("analyzeHint")}
            </p>
            <div className="flex justify-between md:justify-end px-1"></div>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div className="lg:col-span-8 space-y-6">
            <div className="flex justify-between items-end border-b border-white/5 pb-2">
              <div className="flex items-baseline gap-4">
                <h2
                  className="text-3xl font-serif text-foreground flex items-center gap-2"
                  data-testid="text-symbol-name"
                >
            ...
            {stockLoading ? (
              ...
              <div className="h-[500px] flex flex-col items-center justify-center bg-card/20 rounded-lg border border-red-500/20 border-dashed text-muted-foreground font-light">
                ...
                <p className="text-red-400">
                  {t("failedToLoad")} {symbol}
                </p>
                <p className="text-xs text-muted-foreground mt-2">
                  {t("tryDifferentSymbol")}
                </p>
              </div>
            ...
            {analysisLoading ? (
              ...
              <div className="glass-panel p-4 rounded-lg border border-primary/10">
                ...
                <ul
                  className="text-sm text-foreground/90 font-light leading-relaxed space-y-1.5 list-none"
                  data-testid="text-pattern-analysis-below-chart"
                >
                  {analysis.patternAnalysis
                    .split(/(?<=[.!?])\s+/)
          ...
          <div className="lg:col-span-4 space-y-8">
            ...
            <div
              className="bg-card/40 backdrop-blur-sm border border-white/[0.06] rounded-2xl p-5 shadow-2xl shadow-black/40"
              data-testid="panel-favourites"
            >
              <div className="flex items-center justify-between mb-3">
                ...
                {canUseFavourites ? (
                  <span
                    ...
                    data-testid="text-favourites-count"
                  >
                    {t("favouritesCount", {
                      count: String(favourites.length),
                      max: String(maxFavourites),
                    })}
                  </span>
                ) : (
              ...
              {!canUseFavourites ? (
                ...
                <p
                  className="text-xs text-muted-foreground/50 font-light py-4 text-center"
                  data-testid="text-no-favourites"
                >
                  {t("noFavouritesYet")}
                </p>
              ) : (
                    ...
                    <div
                      ...
                      className={`flex items-center justify-between py-2 px-3 rounded-lg transition-colors ${
                        fav.symbol === symbol
                          ...
                          : "bg-background/20 border border-transparent hover:border-white/[0.06]"
                      }`}
                    ...
                    >
                      <div className="flex items-center gap-2 min-w-0 flex-1">
                        ...
                        <div className="min-w-0">
                          <span
                            className="text-xs text-primary font-mono block truncate"
                            data-testid={`text-fav-symbol-${fav.id}`}
                          >
                            {fav.symbol}
                          </span>
              ...
              {canUseFavourites && (
                <div
                  className="flex items-center gap-1.5 mt-3 pt-3 border-t border-white/[0.04]"
                  data-testid="text-favourites-email-remark"
                >
                  <Mail className="w-3 h-3 text-primary/40 shrink-0" />
                  <span className="text-[12px] text-muted-foreground/80 leading-tight">
